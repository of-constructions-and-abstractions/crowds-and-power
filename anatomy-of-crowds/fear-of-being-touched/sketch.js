...
function setup() {
  ...
  for (let i = 0; i < numParticles; i++) {
    particles.push(createParticle(random(width), random(height)));
  }
  
  mouseParticle = createParticle(mouseX, mouseY);
...
function draw() {
  ...
  // Update the mouse particle position
  mouseParticle.position.x = mouseX;
  mouseParticle.position.y = mouseY;
  
  // Update and draw the mouse particle
  updateParticle(mouseParticle, createVector(0, 1));
  drawParticle(mouseParticle);
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    let mouseDistance = dist(
      particle.position.x,
      particle.position.y,
      mouseParticle.position.x,
      mouseY
    );
    if (mouseDistance < minDistance) {
      // Calculate repulsion force
      let repelForce = p5.Vector.sub(
        particle.position,
    ...
    updateParticle(particle, repulsionSum)
    drawParticle(particle)
    
  }
}

function updateParticle(particle, repulsionSum) {
  // Update particle position
    ...
    particle.targetHeadPosition = p5.Vector.add(
      particle.headPosition,
      targetHeadOffset
    );
    particle.targetBodyPosition = p5.Vector.add(
      particle.bodyPosition,
      repulsionSum
    );
    ...
    particle.auraSize = lerp(
      particle.auraSize,
      particle.targetAuraSize,
      bodyTransitionDuration
    );
    particle.headPosition = p5.Vector.slerp(
      particle.headPosition,
      particle.targetHeadPosition,
      transitionDuration
    );
    particle.bodyPosition = p5.Vector.slerp(
      particle.bodyPosition,
      particle.targetBodyPosition,
      transitionDuration
    );
}
...
function drawParticle(particle) {
  // Draw particle as a human-like shape
    ...
    fill(0);
    line(
      particle.bodyPosition.x,
      particle.bodyPosition.y,
      particle.bodyPosition.x,
      particle.bodyPosition.y + 20
    ); // Body
    line(
      particle.bodyPosition.x,
      particle.bodyPosition.y + 5,
      particle.bodyPosition.x - 10,
      particle.bodyPosition.y + 15
    ); // Left arm
    line(
      particle.bodyPosition.x,
      particle.bodyPosition.y + 5,
    ...
    if (particle.auraSize >= 85) {
      fill(255);
      stroke(0);
      ellipse(
        particle.headPosition.x + 20,
        particle.headPosition.y - 20,
        80,
        20
      );
      noStroke();
      ...
      text(
        "ew, get away !",
        particle.headPosition.x + 10,
        particle.headPosition.y - 18
      );
    }
